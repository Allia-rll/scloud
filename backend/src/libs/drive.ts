...
async function uploadFile(file: Express.Multer.File, creds: AuthCredentials, part: string []) {
  const auth = getAuth(creds);
...
async function createFolder(name: string, creds: AuthCredentials, par: string[]) {
  const auth = getAuth(creds);
  const drive = google.drive({ version: "v3", auth });
  const res = await drive.files.create({
    requestBody: {
