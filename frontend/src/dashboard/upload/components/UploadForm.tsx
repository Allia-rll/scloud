...
export default function UploadForm({ onFileUploaded, file }: UploadFormProps) {
  ...
  const onSubmit = async (data: FileInput) => {
    try {
      data.codproyecto = projects.find(p => p.nombpyto === data.codproyecto)?.codpyto || "";
    ...
    const fetchProject = async () => {
      try {
        const response = await fetch(
          `http://localhost:5050/api/v1/proyectos/${cia.codcia}`
        );
